...
def load_resources():
    ...
    with st.spinner("🔍 Loading reports"):
     # Read the pre-loaded files (already converted to markdown so save some time during workshop - ~30sec-1min per pdf)
        ...
        with open("reports/GS12817.md", "r", encoding="utf-8") as f:
            GS12817 = f.read()

        with open("reports/GS12533.md", "r", encoding="utf-8") as f:
            GS12533 = f.read()

        with open("reports/GS12570.md", "r", encoding="utf-8") as f:
            GS12570 = f.read()
        st.session_state["project_dict_actors"] = {
            'VCS2410': VCS2410, 
            'VCS2834': VCS2834,
            'GCSP1025': GCSP1025
        }

        st.session_state["project_dict_cookstoves"]={
                'GS12533': GS12533,
                'GS12570': GS12570,
                'GS12817': GS12817
            }

        st.session_state["user_prompt_actors"] = """
        **Example User Prompt**:
        user_prompt =  
        <objective>
        ...
        """
        st.session_state['output_structure_actors'] = """
  ...
  """
        st.session_state["user_prompt_cookstoves"] = """ 
            ** Your Task: Extract information from carbon project descriptions**

            1. Methods of emission reduction:
            The method to be used in a carbon project to reduce emissions from cookstoves depends on project characteristics.  Choose from the following options:

            - Methods 1: Baseline and project fuel(s) are identical and emission
            reductions are exclusively from improved efficiency
            - Method 2: Baseline and project fuel(s) are identical, emission reductions
            are exclusively from improved efficiency, and the default baseline fuel
            consumption is applied
            - Method 3: Baseline and project fuel(s) are not identical and emission
            reductions are from fuel switch and efficiency gains
            - Other: Other method (please specify)


            2. Feedstocks (both project and baseline):
            Classify the fuel used in the cookstoves in the project scenario and the one used in the baseline scenario.

            The feedstock is the type of fuel used in the cookstoves. Choose from the following options:
            - woody biomass
            - general biomass
            - solid fossil fuels
            - liquid fossil fuels
            ...
            """
        st.session_state['system_prompt'] = "You are a research assistant. Base all your answers on information carbon project descriptions. Do not add any information that is not in the project description. If you are not sure, write 'Not found'."
        st.session_state['output_structure_cookstoves'] =""" 
                ...
                "Output": 
                        {
                            ...
                            "quote_method": "quote from text that supports claim",
                            "Feedstock_baseline": "liquid fossil fuels",
                            "quote_feedstock_baseline": "quote from text that supports claim",
                            "Feedstock_project": "Solar cooking (electric with PV)",
                            "quote_feedstock_project": "quote from text that supports claim"
...
def task_1():
    ...
    **Length of Documents (PDFs include images, tables, etc.):**
    ...
    st.markdown("""
                ...
                """)
    st.download_button(
        label="Download GS12533 PDF",
        data=open("reports/Beispiele_Cookstoves/GS12533.pdf", "rb").read(),
        file_name="GS12533.pdf",
        mime="application/pdf"
    )
    st.download_button(
        label="Download GS12570 PDF",
        data=open("reports/Beispiele_Cookstoves/GS12570.pdf", "rb").read(),
        file_name="GS12570.pdf",
        mime="application/pdf"
    )
    st.download_button(
        label="Download GS12817 PDF",
        data=open("reports/Beispiele_Cookstoves/GS12817.pdf", "rb").read(),
        file_name="GS12817.pdf",
        mime="application/pdf"
    )

    # Pricing Table (May 2025)
    pricing = { 
        "gpt 5": {"input": 1.250, "output": 2.0},
        "gp 5-mini": {"input": 0.25, "output": 1.6},
    ...
    project_dict = st.session_state["project_dict_cookstoves"]
...
# Streamlit app for Task 2 - Analyzing PDFs using LLM API
def task_2():
    st.header("📝 Task 2: System Prompts and Temperature")
...
def task_3():
    st.header("📝 Task 3: Extracting Information from Documents")

    # Instructions
    st.markdown("""
    The goal of this task is to information from unstructured text, specifically the specificities of cookstove projects.

    ### Exercise:
    1. Write a user prompt that tells the model what to do (e.g., extracting information on cookstoves).

    To help the model understand what to do you need to provide a system prompt. Be precise and provide enough information so the model can understand what to do.
    """)
    ...
    with st.expander("Click to see Information to be extracted"):
        ...
        **Information to be extracted from the PDD**:
        1. Methods of emission reduction according to the Methodology.\n There are three methods for emission reduction:
            ...
            - Method 3: Baseline and project fuel(s) are not identical and emission reductions are from fuel switch and efficiency gains
    ...
    if "project_dict_cookstoves" not in st.session_state:
    ...
    project_choice = st.selectbox(
        ...
        options=list(st.session_state["project_dict_cookstoves"].keys())
    )

    # Retrieve the chosen project description
    project_description = st.session_state["project_dict_cookstoves"][project_choice]

    # User prompt input (extraction prompt for companies)
    st.subheader("User Prompt (Tell the model what to do)")
    user_prompt = st.text_area(
        "Write a prompt to instruct the model to extract relevant structured information (e.g., calculation method and project & baseline feedstock). The text of the project description will be added automatically to your prompt.",
...
def task_4():
    ...
    output_structure = st.session_state['output_structure_cookstoves']
    ...
    if st.button("Generate Response"):
        if user_prompt:
            ...
            try:
                ...
                df_actors = pd.json_normalize(data['Output'])
...
def task_5():
    ...
    st.markdown("""
    As seen before, some important information might be hidden in **images** in project descriptions.  
    ...
    if "project_dict_actors" not in st.session_state:
        st.error("Session state with project descriptions not found.")
        return

    project_dict = st.session_state["project_dict_actors"]
    ...
    output_structure = st.session_state['output_structure_actors']
    full_prompt = st.session_state["user_prompt_actors"] + st.session_state['category_description'] + f" The carbon project description starts here: \n {project_description}" + output_structure
